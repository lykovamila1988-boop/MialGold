...
- ПЕРЕД любым предложением «что автоматизировать / улучшить» ОБЯЗАТЕЛЬНО вызови list_automations
  и сверься: НЕ предлагай построить то, что уже есть (n8n-воркфлоу, tools-скрипты, endpoints моста).
  Уже реализовано: еженедельный KPI/дайджест, алерты ошибок, healthcheck, публикация по расписанию,
  захват лидов, измерение охвата 48ч, конкурентная разведка, авто-очистка отчётов. Предлагай ТОЛЬКО
  новое; если всё ключевое покрыто — так и скажи и предложи улучшить существующее.
...
_ALL_AGENTS = ("marina", "victoria", "alina", "dima", "tyoma", "olya",
               "vasya", "lera", "manager", "producer", "rita")


def user_activity(days=7):
    """Как Людмила РЕАЛЬНО пользуется офисом: лог logs/user_activity.jsonl.
    Даёт паттерны для улучшений приложения — кем пользуются, кого игнорируют,
    где ошибки, как долго ждёт ответа, какие темы запросов."""
    f = MILA_FOLDER / "logs" / "user_activity.jsonl"
    try:
        days = int(days)
    except (ValueError, TypeError):
        days = 7
    ...
    try:
        ...
        return json.dumps({"note": "Лог активности пуст — пользователь ещё не писал агентам "
                           "(или logs/user_activity.jsonl не создан). Улучшения предлагай "
                           "осторожно, помечая как гипотезы."}, ensure_ascii=False)

    per_agent, errors, samples = {}, 0, []
    total, resp_times = 0, []
    for ln in lines:
        try:
            r = json.loads(ln)
            ...
            continue
        ts = _safe_dt(r.get("ts", ""))
        if ts and ts < cutoff:
            continue
        total += 1
        a = r.get("agent", "?")
        d = per_agent.setdefault(a, {"count": 0, "errors": 0})
        d["count"] += 1
        if not r.get("ok", True):
            d["errors"] += 1; errors += 1
        if r.get("response_ms"):
            resp_times.append(r["response_ms"])
        if len(samples) < 15 and r.get("message"):
            samples.append({"agent": a, "msg": r["message"][:120]})

    used = set(per_agent)
    ignored = [a for a in _ALL_AGENTS if a not in used]
    top = sorted(per_agent.items(), key=lambda kv: kv[1]["count"], reverse=True)
    return json.dumps({
        "period_days": days,
        "total_messages": total,
        "by_agent": {a: d for a, d in top},
        "ignored_agents": ignored,        # агенты, которыми НЕ пользуются — кандидаты на доработку/удаление
        "error_count": errors,
        "avg_response_ms": round(sum(resp_times) / len(resp_times)) if resp_times else None,
        "recent_requests": samples,       # темы запросов — что реально нужно пользователю
        "hint": "Предлагай улучшения приложения от ЭТИХ данных: усилить часто используемых "
                "агентов, разобраться почему игнорируют других, убрать трение там где ошибки/долгий ответ.",
    }, ensure_ascii=False, indent=2)


def list_automations():
    """Инвентарь УЖЕ существующих автоматизаций: n8n-воркфлоу, tools-скрипты,
    endpoints моста. Стас обязан вызвать это ПЕРЕД тем как предлагать «что
    автоматизировать» — иначе предлагает построить готовое (частая ошибка)."""
    wf_dir = MILA_FOLDER / "n8n" / "workflows"
    workflows = []
    if wf_dir.exists():
    ...
    auto_scripts = []
    if tools_dir.exists():
        kw = ("weekly", "alert", "cleanup", "kpi", "digest", "lead", "publish",
              "healthcheck", "notify", "poller", "gumroad", "calcom", "log_rotate")
        for f in sorted(tools_dir.glob("*.py")):
            if any(k in f.name.lower() for k in kw):
                auto_scripts.append(f.name)
    bridge = MILA_FOLDER / "mila-office" / "n8n_bridge.py"
    endpoints = []
    if bridge.exists():
        endpoints = re.findall(r'@app\.post\("(/v1/[^"]+)"\)',
                               bridge.read_text(encoding="utf-8"))
    return json.dumps({
        "note": "Это УЖЕ автоматизировано. Предлагай ТОЛЬКО то, чего здесь нет.",
        "n8n_workflows": workflows,
        "tools_scripts": auto_scripts,
...
TOOLS = core_tools("Прочитать лог/отчёт/файл рабочей папки",
    ...
    {"name": "list_automations",
     "description": "Инвентарь УЖЕ существующих автоматизаций (n8n-воркфлоу, tools-скрипты, "
                    "endpoints моста). ОБЯЗАТЕЛЬНО вызови ПЕРЕД предложениями «что автоматизировать», "
                    "чтобы не предлагать построить готовое.",
     "input_schema": {"type": "object", "properties": {}}},
    {"name": "user_activity",
     "description": "Как пользователь РЕАЛЬНО пользуется приложением (лог чата): по агентам, "
                    "игнорируемые агенты, ошибки, время ответа, темы запросов. Вызови, чтобы "
                    "предлагать улучшения приложения ОТ ПОВЕДЕНИЯ, а не из головы.",
     "input_schema": {"type": "object", "properties": {
...
def handle(name, inp):
    ...
    if name == "user_activity":
        return user_activity(inp.get("days", 7))
...
QUICK = {
    ...
    "/автоматизировать": "СНАЧАЛА вызови list_automations и посмотри, что УЖЕ автоматизировано "
                         "(n8n-воркфлоу, tools-скрипты, endpoints моста). Предложи 2-3 НОВЫЕ автоматизации, "
                         "которых там НЕТ — с критерием успеха и оценкой времени. Если всё ключевое уже "
                         "покрыто — честно скажи это и предложи улучшить существующее, а не дублировать.",
    "/улучшения":  "Сделай app_review И вызови list_automations. Предложи топ-3 технических улучшения, "
                   "которых ещё нет (не предлагай построить уже существующее). Приоритеты + критерий приёмки.",
    "/активность": "Вызови user_activity и разбери, как Людмила реально пользуется офисом: кем "
                   "пользуется чаще, кого игнорирует, где ошибки/долгий ответ, какие темы запросов. "
                   "Предложи топ-3 улучшения приложения ОТ этих данных — с критерием успеха.",
}
